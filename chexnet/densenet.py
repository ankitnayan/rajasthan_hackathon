...
class DenseNet:
    ...
    def save_path(self):
        try:
            ...
            save_path = 'save_60k/%s' % self.model_identifier
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("Initial_convolution"):
            output = self.conv2d(self.images, out_features=self.first_output_features,kernel_size=7, strides=[1, 2, 2, 1])
        ...
        for block in range(self.total_blocks):
            with tf.variable_scope("Block_%d" % block):
                ...
                print ("#layes in this block: ", layers_per_block)
                print ("Input shape (into block): ", tf.shape(output))
                output = self.add_block(output, growth_rate, layers_per_block)
                print ("Output shape (from block): ", tf.shape(output))
                print ("*****************  END OF BLOCK *************************")
        ...
        self.argmax = tf.argmax(prediction, 1)
    ...
    def predict(self, images):
        ...
        fetches = [self.last_block_output, self.prediction, self.argmax]
